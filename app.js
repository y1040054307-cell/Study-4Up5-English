(() => {
  ...
  const PARENT_STORE = `${STORE}:parent-pin-hash`;
  const USER_PROFILES = [
  ...
  const load = (profileId=activeUserId) => { try {
    ...
    return {...defaultState,...raw,suns:TEST_MODE?TEST_BALANCE:persistedEconomy.suns,foods:TEST_MODE?TEST_BALANCE:persistedEconomy.foods,evidence:{...(raw.evidence||{})},recordingProgress:{...(raw.recordingProgress||{})},reviewSchedule:{...(raw.reviewSchedule||{})},plant,pets,quiz:{...defaultState.quiz,...(raw.quiz||{})},beginner,grammar,abilities,papers,bridge};
  ...
  let speakingPlayedKeys = new Set();
  let lessonRecorder = null;
  let lessonRecordingStream = null;
  let lessonRecordingChunks = [];
  let lessonRecordingButton = null;
  let lessonRecordingStartedAt = 0;
  const lessonRecordingUrls = new Map();
  let phonemePlayedKeys = new Set();
  ...
  const versionStatus = book => book.version?.status || "reference";
  const courseContentLabel = book => versionStatus(book)==="aligned"?"原创同步学习课件":versionStatus(book)==="precheck"?"新版主题预校对课件":"原创英语主题练习";
  const versionBadge = (book, compact=false) => `<span class="course-version-badge version-${versionStatus(book)}">${esc(compact?(book.version?.short||book.edition):(book.version?.label||book.edition))}</span>`;
  const versionDetails = book => `<div class="course-version-details version-${versionStatus(book)}"><div>${versionBadge(book)}<strong>${esc(book.version?.statusLabel||"请核对版本")}</strong></div><p>${esc(book.version?.note||"请以学校实际发放的纸质教材为准。")}</p><small>本站内容属性：原创讲解与练习，不是出版社教材原文或官方电子课件。</small></div>`;
  ...
  const GRAMMAR_EXTRA_QUESTIONS={
    articles:[{q:"It takes me ___ hour to finish it.",options:["a","an","the"],answer:"an",explain:"hour 的 h 不发音，开头是元音音素。"},{q:"Please open ___ book on your desk.",options:["a","an","the"],answer:"the",explain:"双方知道是桌上的那本书，用 the。"},{q:"She has ___ umbrella and ___ raincoat.",options:["a; a","an; a","an; an"],answer:"an; a",explain:"umbrella 以元音音素开头，raincoat 以辅音音素开头。"},{q:"___ children in this photo are my classmates.",options:["A","The","An"],answer:"The",explain:"in this photo 限定了具体哪些孩子。"}],
    nouns:[{q:"Two ___ are playing under the tree.",options:["child","childs","children"],answer:"children",explain:"child 的不规则复数是 children。"},{q:"We need two ___ of bread.",options:["piece","pieces","breads"],answer:"pieces",explain:"bread 不可数，two 后量词 piece 用复数。"},{q:"There ___ some milk in the glass.",options:["is","are","am"],answer:"is",explain:"milk 是不可数名词，谓语按单数。"},{q:"I can see five ___ on the farm.",options:["sheep","sheeps","sheepes"],answer:"sheep",explain:"sheep 单复数同形。"}],
    verbs:[{q:"Amy ___ her homework now.",options:["does","is doing","did"],answer:"is doing",explain:"now 提示现在进行时，用 is doing。"},{q:"They ___ to Xiamen tomorrow.",options:["will go","went","goes"],answer:"will go",explain:"tomorrow 提示一般将来时。"},{q:"Did Ben ___ the film yesterday?",options:["watched","watch","watches"],answer:"watch",explain:"Did 后实义动词回到原形。"},{q:"Each student ___ a book.",options:["have","has","having"],answer:"has",explain:"each student 作第三人称单数。"}],
    prepositions:[{q:"My birthday is ___ May 8th.",options:["at","on","in"],answer:"on",explain:"具体日期前用 on。"},{q:"Fujian is ___ China.",options:["in","on","at"],answer:"in",explain:"在国家内部用 in。"},{q:"Look ___ the picture carefully.",options:["to","at","for"],answer:"at",explain:"look at 是固定搭配。"},{q:"This present is ___ you.",options:["from","for","of"],answer:"for",explain:"for 可表示给某人。"}],
    pronouns:[{q:"Tom and I are friends. ___ study together.",options:["We","Us","Our"],answer:"We",explain:"句首作主语用主格 We。"},{q:"These books are ___.",options:["our","ours","us"],answer:"ours",explain:"空格后没有名词，用名词性物主代词 ours。"},{q:"I can see Amy. Please call ___.",options:["she","her","hers"],answer:"her",explain:"call 后作宾语用 her。"},{q:"Is there ___ in the box?",options:["anything","somebody","nothing"],answer:"anything",explain:"一般疑问句中表示任何东西常用 anything。"}],
    adjectives:[{q:"The turtle walks ___.",options:["slow","slowly","slower"],answer:"slowly",explain:"修饰动作 walks 用副词 slowly。"},{q:"This question is ___ than that one.",options:["easy","easier","easiest"],answer:"easier",explain:"than 提示比较级。"},{q:"Mount Qomolangma is ___ mountain in the world.",options:["high","higher","the highest"],answer:"the highest",explain:"在全世界范围比较用最高级。"},{q:"She sings very ___.",options:["good","well","best"],answer:"well",explain:"修饰 sings 用副词 well。"}],
    sentence:[{q:"选择正确语序。",options:["Where you do live?","Where do you live?","Do where you live?"],answer:"Where do you live?",explain:"特殊疑问词后接一般疑问句语序。"},{q:"___ they playing football now?",options:["Do","Are","Is"],answer:"Are",explain:"现在进行时把 are 提到主语前。"},{q:"She ___ like rainy days.",options:["don't","doesn't","isn't"],answer:"doesn't",explain:"第三人称单数实义动词否定用 doesn't。"},{q:"选择正确语序。",options:["He goes to school by bus every day.","He every day by bus goes school.","Goes he school every day by bus."],answer:"He goes to school by bus every day.",explain:"主语和谓语在前，方式与时间放后。"}],
    clauses:[{q:"Would you like tea ___ milk?",options:["or","so","because"],answer:"or",explain:"or 表示二选一。"},{q:"It was late, ___ we went home.",options:["so","but","because"],answer:"so",explain:"前面是原因，so 引出结果。"},{q:"Call me ___ you arrive.",options:["when","but","or"],answer:"when",explain:"when 引出到达的时间。"},{q:"The boy ___ is running is Tom.",options:["who","where","what"],answer:"who",explain:"who 引导定语从句修饰表示人的 boy。"}]
  };
  ...
  const PAPER_TYPES = [
    {id:"fujian-unit",icon:"🏫",kind:"福建公开卷题型参考",title:"本单元综合测评卷",level:"综合",minutes:60,count:50,pass:40,desc:"原创组卷：参考福建小学公开试卷常见结构，覆盖听力、词汇、易混词、句型、情境、阅读与综合运用。"}
  ];
  ...
  const WORD_PROFILE_OVERRIDES={
    light:{pos:"名词 / 形容词 / 动词",forms:"名词单数 light；复数 lights；形容词无单复数"},in:{pos:"介词 / 副词 / 形容词",forms:"无单复数变化"},right:{pos:"名词 / 形容词 / 副词",forms:"名词单数 right；复数 rights"},left:{pos:"名词 / 形容词 / 副词 / leave的过去式",forms:"名词单数 left；复数 lefts（少用）"},kind:{pos:"形容词 / 名词",forms:"名词单数 kind；复数 kinds"},watch:{pos:"动词 / 名词",forms:"名词单数 watch；复数 watches；动词三单 watches"},play:{pos:"动词 / 名词",forms:"名词单数 play；复数 plays；动词三单 plays"},call:{pos:"动词 / 名词",forms:"名词单数 call；复数 calls；动词三单 calls"},change:{pos:"动词 / 名词",forms:"名词单数 change；复数 changes；动词三单 changes"},fit:{pos:"动词 / 形容词",forms:"无名词单复数；动词三单 fits"},orange:{pos:"名词 / 形容词",forms:"名词单数 orange；复数 oranges"},fish:{pos:"名词 / 动词",forms:"名词单数 fish；复数 fish（常用）/ fishes（强调种类）"},water:{pos:"不可数名词 / 动词",forms:"作“水”通常无复数；动词三单 waters"},work:{pos:"不可数名词 / 动词",forms:"表示工作时通常无复数；动词三单 works"},rest:{pos:"名词 / 动词",forms:"名词单数 rest；复数 rests；动词三单 rests"},plan:{pos:"名词 / 动词",forms:"名词单数 plan；复数 plans；动词三单 plans"},train:{pos:"名词 / 动词",forms:"名词单数 train；复数 trains；动词三单 trains"},book:{pos:"名词 / 动词",forms:"名词单数 book；复数 books；动词三单 books"},class:{pos:"名词",forms:"单数 class；复数 classes"},present:{pos:"名词 / 形容词 / 动词",forms:"名词单数 present；复数 presents"},spring:{pos:"名词 / 动词",forms:"名词单数 spring；复数 springs"},park:{pos:"名词 / 动词",forms:"名词单数 park；复数 parks；动词三单 parks"},show:{pos:"动词 / 名词",forms:"名词单数 show；复数 shows；动词三单 shows"},matter:{pos:"名词 / 动词",forms:"名词单数 matter；复数 matters；动词三单 matters"},dream:{pos:"名词 / 动词",forms:"名词单数 dream；复数 dreams；动词三单 dreams"},miss:{pos:"动词 / 名词",forms:"动词三单 misses；Miss 可作女性称谓"},wish:{pos:"名词 / 动词",forms:"名词单数 wish；复数 wishes；动词三单 wishes"},help:{pos:"名词 / 动词",forms:"作“帮助”时通常不可数；动词三单 helps"},cook:{pos:"名词 / 动词",forms:"名词 cook 指厨师，复数 cooks；动词三单 cooks"},dance:{pos:"名词 / 动词",forms:"名词单数 dance；复数 dances；动词三单 dances"},exercise:{pos:"名词 / 动词",forms:"指锻炼时常不可数；指练习题时可数 exercises；动词三单 exercises"},race:{pos:"名词 / 动词",forms:"名词单数 race；复数 races；动词三单 races"},floor:{pos:"名词",forms:"单数 floor；复数 floors"},date:{pos:"名词 / 动词",forms:"名词单数 date；复数 dates；动词三单 dates"},view:{pos:"名词 / 动词",forms:"名词单数 view；复数 views；动词三单 views"},space:{pos:"名词",forms:"表示太空通常不可数；表示空位时可数，复数 spaces"},
    chicken:{pos:"可数名词 / 不可数名词",forms:"指鸡时：a chicken / chickens；指鸡肉时不可数"},family:{pos:"可数名词",forms:"单数 family；复数 families；指家人整体时注意主谓意义"},cold:{pos:"形容词 / 名词",forms:"形容词无单复数；a cold 表示一次感冒，复数 colds"},glass:{pos:"可数 / 不可数名词",forms:"玻璃材料不可数；玻璃杯可数，复数 glasses"},paper:{pos:"可数 / 不可数名词",forms:"纸张通常不可数；试卷、论文可数，复数 papers"},homework:{pos:"不可数名词",forms:"通常不用复数，不说 homeworks；可说 some homework"},child:{pos:"可数名词",forms:"单数 child；不规则复数 children"},person:{pos:"可数名词",forms:"单数 person；常用复数 people"},man:{pos:"可数名词",forms:"单数 man；不规则复数 men"},woman:{pos:"可数名词",forms:"单数 woman；不规则复数 women"},mouse:{pos:"可数名词",forms:"单数 mouse；不规则复数 mice"},foot:{pos:"可数名词",forms:"单数 foot；不规则复数 feet"},tooth:{pos:"可数名词",forms:"单数 tooth；不规则复数 teeth"},sheep:{pos:"可数名词",forms:"单复数同形：one sheep / two sheep"},photo:{pos:"可数名词",forms:"单数 photo；复数 photos"},clothes:{pos:"复数名词",forms:"通常只用复数 clothes；单件衣物可说 an item of clothing"},trousers:{pos:"复数名词",forms:"通常说 a pair of trousers"},glasses:{pos:"复数名词",forms:"指眼镜时说 a pair of glasses"},dishes:{pos:"复数名词",forms:"单数 dish；复数 dishes"},
    hello:{pos:"感叹词 / 问候语",forms:"固定问候语，没有单复数变化"},name:{pos:"可数名词 / 动词",forms:"名词单数 name；复数 names；动词三单 names"},friend:{pos:"可数名词",forms:"单数 friend；复数 friends"},meet:{pos:"动词",forms:"动词原形 meet；第三人称单数 meets；过去式 met"},please:{pos:"礼貌用语 / 副词 / 动词",forms:"作礼貌用语时没有单复数变化；动词三单 pleases"},fine:{pos:"形容词 / 名词 / 动词",forms:"作“很好”时是形容词，没有单复数变化"},goodbye:{pos:"感叹词 / 名词",forms:"作告别语时没有单复数变化"},day:{pos:"可数名词",forms:"单数 day；复数 days"},
    one:{pos:"基数词 / 代词",forms:"数词没有单复数变化"},two:{pos:"基数词",forms:"数词没有单复数变化"},three:{pos:"基数词",forms:"数词没有单复数变化"},four:{pos:"基数词",forms:"数词没有单复数变化"},five:{pos:"基数词",forms:"数词没有单复数变化"},six:{pos:"基数词",forms:"数词没有单复数变化"},seven:{pos:"基数词",forms:"数词没有单复数变化"},eight:{pos:"基数词",forms:"数词没有单复数变化"},nine:{pos:"基数词",forms:"数词没有单复数变化"},ten:{pos:"基数词",forms:"数词没有单复数变化"},
    red:{pos:"名词 / 形容词",forms:"作颜色名词或形容词时通常没有单复数变化"},blue:{pos:"名词 / 形容词",forms:"作颜色名词或形容词时通常没有单复数变化"},green:{pos:"名词 / 形容词",forms:"作颜色名词或形容词时通常没有单复数变化"},yellow:{pos:"名词 / 形容词",forms:"作颜色名词或形容词时通常没有单复数变化"},black:{pos:"名词 / 形容词",forms:"作颜色名词或形容词时通常没有单复数变化"},white:{pos:"名词 / 形容词",forms:"作颜色名词或形容词时通常没有单复数变化"},pink:{pos:"名词 / 形容词",forms:"作颜色名词或形容词时通常没有单复数变化"},color:{pos:"可数名词 / 动词",forms:"名词单数 color；复数 colors；动词三单 colors"},colorful:{pos:"形容词",forms:"形容词没有单复数变化"},
    father:{pos:"可数名词",forms:"单数 father；复数 fathers"},mother:{pos:"可数名词",forms:"单数 mother；复数 mothers"},brother:{pos:"可数名词",forms:"单数 brother；复数 brothers"},sister:{pos:"可数名词",forms:"单数 sister；复数 sisters"},grandpa:{pos:"可数名词",forms:"单数 grandpa；复数 grandpas"},grandma:{pos:"可数名词",forms:"单数 grandma；复数 grandmas"},love:{pos:"名词 / 动词",forms:"作“爱”时名词通常不可数；动词三单 loves"},he:{pos:"人称代词（主格）",forms:"第三人称男性单数；宾格 him；形容词性物主代词 his"},she:{pos:"人称代词（主格）",forms:"第三人称女性单数；宾格 her；形容词性物主代词 her"}
  };
  ...
  const lessonRecordKey = token => `${unitKey()}:${token}`;
  const lessonRecordCount = token => Number(state.recordingProgress?.[lessonRecordKey(token)]||0);
  function lessonRecordButton(token,required,label,minMs=700){
    const count=lessonRecordCount(token),met=count>=required;
    return `<div class="lesson-record-row ${met?"met":""}" data-record-token="${esc(token)}"><button type="button" class="lesson-record-button" data-lesson-record="${esc(token)}" data-record-required="${required}" data-record-min-ms="${minMs}" data-record-label="${esc(label)}">🎙️ ${met?"继续巩固":"录音跟读"}</button><span class="lesson-record-count">${Math.min(count,required)}/${required}次</span><div class="lesson-record-result"><small>${met?"✓ 已达到跟读要求，可继续录音巩固。":"录音只保留在当前页面，不会上传。"}</small></div></div>`;
  }
  function wordRecordingStatus(u){
    const words=u.core.slice(0,Math.min(8,u.core.length));let done=0,total=0,missing=[];
    words.forEach((word,index)=>{const wordToken=`word:${word.word}`,wordCount=lessonRecordCount(wordToken);total+=2;done+=Math.min(wordCount,2);if(wordCount<2)missing.push(`${word.word}单词${2-wordCount}次`);wordSentenceStudies(word,index,u).slice(0,3).forEach((_,sentenceIndex)=>{const token=`word-example:${word.word}:${sentenceIndex}`,count=lessonRecordCount(token);total+=1;done+=Math.min(count,1);if(count<1)missing.push(`${word.word}例句${sentenceIndex+1}`);});});
    return{done,total,missing,passed:done>=total};
  }
  function patternRecordingStatus(u){
    const patterns=expandedPatternItems(u);let done=0,total=patterns.length*3,missing=[];
    patterns.forEach((pattern,index)=>{const token=`pattern:${index}:${pattern.en}`,count=lessonRecordCount(token);done+=Math.min(count,3);if(count<3)missing.push(`句型${index+1}还差${3-count}次`);});
    return{done,total,missing,passed:done>=total};
  }
  function refreshLessonRecordingOverview(){const host=$("lessonContent")?.querySelector(".lesson-record-overview");if(!host)return;const progress=state.stage==="words"?wordRecordingStatus(unitNow()):state.stage==="patterns"?patternRecordingStatus(unitNow()):null;if(!progress)return;const text=host.querySelector("b"),bar=host.querySelector("progress");if(text)text.textContent=`${progress.done}/${progress.total} 次`;if(bar){bar.max=progress.total;bar.value=progress.done;}host.classList.toggle("complete",progress.passed);}
  async function startLessonRecording(button){
    if(lessonRecorder?.state==="recording"){if(lessonRecordingButton===button)lessonRecorder.stop();else toast("请先停止当前录音，再开始下一项");return;}
    if(!navigator.mediaDevices?.getUserMedia||!window.MediaRecorder)return toast("当前浏览器不支持网页录音，请使用最新版微信、Chrome、Edge 或 Safari");
    try{
      lessonRecordingStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true}});lessonRecordingChunks=[];lessonRecordingButton=button;lessonRecordingStartedAt=Date.now();
      const preferred=["audio/webm;codecs=opus","audio/mp4","audio/webm"].find(type=>MediaRecorder.isTypeSupported?.(type));lessonRecorder=new MediaRecorder(lessonRecordingStream,preferred?{mimeType:preferred}:undefined);
      const row=button.closest(".lesson-record-row"),result=row?.querySelector(".lesson-record-result");button.textContent="■ 读完后停止";button.classList.add("recording");if(result)result.innerHTML="<b>正在录音……请清楚读完，再点击停止。</b>";
      lessonRecorder.ondataavailable=event=>{if(event.data.size)lessonRecordingChunks.push(event.data);};
      lessonRecorder.onstop=()=>{const elapsed=Date.now()-lessonRecordingStartedAt,minMs=Number(button.dataset.recordMinMs||700),token=button.dataset.lessonRecord,required=Number(button.dataset.recordRequired||1),label=button.dataset.recordLabel||"本项";lessonRecordingStream?.getTracks().forEach(track=>track.stop());lessonRecordingStream=null;button.classList.remove("recording");lessonRecordingButton=null;
        if(elapsed<minMs){button.textContent="🎙️ 重新录音";if(result)result.innerHTML=`<small>录音太短，没有计入次数。请完整读出${esc(label)}。</small>`;return;}
        const blob=new Blob(lessonRecordingChunks,{type:lessonRecorder.mimeType||"audio/webm"}),oldUrl=lessonRecordingUrls.get(token);if(oldUrl)URL.revokeObjectURL(oldUrl);const url=URL.createObjectURL(blob);lessonRecordingUrls.set(token,url);state.recordingProgress[lessonRecordKey(token)]=lessonRecordCount(token)+1;save();refreshLessonRecordingOverview();const count=lessonRecordCount(token),met=count>=required;row?.classList.toggle("met",met);const counter=row?.querySelector(".lesson-record-count");if(counter)counter.textContent=`${Math.min(count,required)}/${required}次`;button.textContent=`🎙️ ${met?"继续巩固":"再录一次"}`;if(result)result.innerHTML=`<audio controls playsinline src="${url}"></audio><small>${met?"✓ 已达到要求。":"请回听并与标准音比较，再完成下一次。"}</small>`;
      };
      lessonRecorder.start();
    }catch(error){lessonRecordingStream?.getTracks().forEach(track=>track.stop());lessonRecordingStream=null;lessonRecordingButton=null;toast(error?.name==="NotAllowedError"?"需要允许麦克风，才能完成录音跟读":"录音启动失败，请检查麦克风后重试");}
  }
  function bindLessonRecorders(box){box.querySelectorAll("[data-lesson-record]").forEach(button=>button.onclick=()=>startLessonRecording(button));}
  ...
  const carePlant = () => { const n=daysBetween(state.plant.lastDate,iso()); if(n>0){ const progress=plantProgress(),decayDays=Math.max(0,n-2);progress.energy=Math.max(0,progress.energy-decayDays*5);state.plant.lastDate=iso();save(); } };
  ...
  function dateAfter(days){const date=new Date();date.setDate(date.getDate()+days);return iso(date);}
  function scheduleReview(key,correct,mode="practice"){
    const previous=state.reviewSchedule[key]||{level:0,due:iso(),correct:0,wrong:0,modes:{}},level=correct?Math.min(reviewIntervals.length,Number(previous.level||0)+1):0,delay=correct?reviewIntervals[Math.max(0,level-1)]:1;
    state.reviewSchedule[key]={...previous,level,due:dateAfter(delay),last:iso(),correct:Number(previous.correct||0)+(correct?1:0),wrong:Number(previous.wrong||0)+(correct?0:1),modes:{...(previous.modes||{}),[mode]:iso()}};
  }
  const pastLearnedWords = () => {
    ...
    return [...unique.values()].sort((a,b)=>String(state.reviewSchedule[`${a.unitId}:${a.word}`]?.due||"0000").localeCompare(String(state.reviewSchedule[`${b.unitId}:${b.word}`]?.due||"0000")));
  };
  const dueReviewWords=()=>pastLearnedWords().filter(item=>!state.reviewSchedule[`${item.unitId}:${item.word}`]?.due||state.reviewSchedule[`${item.unitId}:${item.word}`].due<=iso());
  ...
  async function hashParentPin(pin){const bytes=new TextEncoder().encode(`sunny-parent:${pin}`);if(crypto.subtle){const digest=await crypto.subtle.digest("SHA-256",bytes);return [...new Uint8Array(digest)].map(value=>value.toString(16).padStart(2,"0")).join("");}return btoa(`sunny-parent:${pin}`);}
  function openParentDialog(){closeProfileDialog();$("parentManagerPanel").hidden=true;$("parentUnlock").hidden=false;$("parentPinInput").value="";$("parentUnlockHint").textContent=localStorage.getItem(PARENT_STORE)?"请输入家长设置的4位数字。":"首次使用：请输入并设置一个4位家长码。";const dialog=$("parentDialog");dialog.showModal?.();dialog.classList.add("open");setTimeout(()=>$("parentPinInput").focus(),80);}
  function closeParentDialog(){const dialog=$("parentDialog");dialog.close?.();dialog.classList.remove("open");}
  async function unlockParentManager(){const pin=$("parentPinInput").value.trim();if(!/^\d{4}$/.test(pin))return toast("请输入4位数字家长码");const hash=await hashParentPin(pin),saved=localStorage.getItem(PARENT_STORE);if(saved&&saved!==hash)return toast("家长码不正确");if(!saved){localStorage.setItem(PARENT_STORE,hash);toast("家长码已设置，请家长记住这4位数字");}$("parentUnlock").hidden=true;$("parentManagerPanel").hidden=false;$("newParentPin").value="";}
  function learningBackupPayload(){save();const profiles={};USER_PROFILES.forEach(profile=>{try{profiles[profile.id]=JSON.parse(localStorage.getItem(profileStoreKey(profile.id))||"{}");}catch{profiles[profile.id]={};}});return{app:"向阳英语成长岛",format:1,exportedAt:new Date().toISOString(),activeUserId,profiles};}
  function downloadLearningBackup(){const payload=learningBackupPayload(),blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=`向阳英语学习备份-${iso()}.json`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast("5个学习身份的数据备份已导出");return payload;}
  async function importLearningBackup(file){if(!file)return;try{const payload=JSON.parse(await file.text());if(payload?.format!==1||!payload.profiles||typeof payload.profiles!=="object")throw new Error("format");if(!confirm("导入会覆盖本浏览器中5个身份的学习数据。系统将先下载当前备份，确定继续吗？"))return;downloadLearningBackup();USER_PROFILES.forEach(profile=>{const data=payload.profiles[profile.id];if(data&&typeof data==="object")localStorage.setItem(profileStoreKey(profile.id),JSON.stringify(data));});activeUserId=validProfileId(payload.activeUserId||activeUserId);localStorage.setItem(ACTIVE_PROFILE_STORE,activeUserId);state=load(activeUserId);renderHeader();route("home");closeParentDialog();toast("学习备份导入成功");}catch{toast("备份文件无法识别，请选择本站导出的JSON文件");}finally{$("importLearningData").value="";}}
  async function saveNewParentPin(){const pin=$("newParentPin").value.trim();if(!/^\d{4}$/.test(pin))return toast("新的家长码必须是4位数字");localStorage.setItem(PARENT_STORE,await hashParentPin(pin));$("newParentPin").value="";toast("家长码已更新");}
  function switchProfile(profileId){
    ...
    selectedGrade=Number(state.bookId[1])||4;selectedTerm=state.bookId.endsWith("a")?"上册":"下册";outlineGrade=selectedGrade;outlineTerm=selectedTerm;beginnerTab="alphabet";paperGrade=selectedGrade;paperTerm=selectedTerm;paperUnitIndex=state.unitIndex;paperActiveId="";paperAnswers={};paperChecked=false;memoryFilter="current";memoryIndex=0;memoryFlipped=false;quizAnswers={};grammarAnswers={};grammarResult=null;diagnosticAnswers={};diagnosticPage=0;abilityListeningAnswers={};abilityListeningChecked=false;abilityPhonicsAnswers={};abilityPhonicsChecked=false;bridgeAbilityAnswers={};bridgeAbilityChecked=false;speakingRecordedKeys=new Set();speakingPlayedKeys=new Set();phonemePlayedKeys=new Set();
  ...
  function route(view){
    ...
    const mobileCore=["home","courses","unit","today","abilities","report"];$("openMoreMenu")?.classList.toggle("active",!mobileCore.includes(view));
  ...
  async function downloadPhonemePack(){const pack=window.PHONEM_AUDIO_PACK,button=$("downloadPhonemePack");if(!pack?.url||!("caches" in window))return toast("当前浏览器不支持音标离线包");button.disabled=true;button.textContent="正在下载音标包……";try{const response=await fetch(pack.url,{cache:"no-store"});if(!response.ok)throw new Error("phoneme");const cache=await caches.open(AUDIO_PACK_CACHE);await cache.put(pack.url,response.clone());phonemeAudioPackBlob=await response.blob();button.textContent="✓ 音标包已下载";toast("48个标准音素已可离线播放");}catch{button.disabled=false;button.textContent="仅下载音标包（2.0 MB）";toast("音标包下载失败，请连接稳定网络后重试");}}

  const bridgeTaskKey=(week,index)=>`w${week}:d${index}`;
  const bridgeDoneCount=()=>state.bridge.done.length;
  ...
  function renderHome(){
    ...
    $("currentCourseCard").innerHTML=`<div class="course-progress-card"><span class="course-icon">${unit.icon}</span><div class="course-main"><div class="home-course-meta"><small>${book.label} · Unit ${unit.number}</small>${versionBadge(book,true)}</div><h3>${unit.title} <i>${unit.zh}</i></h3><p>${unit.goal}</p><div class="thin-bar"><span style="width:${unitDone/stages.length*100}%"></span></div><small>${unitDone}/${stages.length} 个学习环节已完成</small></div><button class="primary" data-open-stage="${nextStage().id}">继续</button></div>`;
  ...
  function renderCourses(){
    ...
    renderBridgeBanner("courseBridgeBanner",selectedGrade===6,"六年级课程");
    const done=book.units.filter(u=>stages.every(s=>state.stageDone.includes(`${u.id}:${s.id}`))).length;
    $("bookSummary").innerHTML=`<div class="book-summary-copy"><div class="book-version-line">${versionBadge(book)}<strong>${esc(book.version.statusLabel)}</strong></div><h2>${book.label}</h2><p>${book.units.length}个主题单元 · 每单元${stages.length}步 · 原创讲解与练习</p><p class="book-version-note">${esc(book.version.note)}</p><small class="original-content-note">本站原创内容，不是出版社教材原文或官方电子课件。</small></div><div class="book-summary-progress"><b>${done}/${book.units.length}</b><small>完成单元</small></div>`;
  ...
  function paperSourceUnits(book,unitIndex,type){
    ...
    if(type.id==="fujian-unit")return [current];
    const previous=paperPreviousUnits(book,unitIndex);
    if(type.id==="bridge-mock")return [...previous,current];
  ...
  function paperQuestionPool(book,unitIndex,type){
    ...
    const advanced=true,wordAt=index=>words[index%words.length],patternAt=index=>patterns[index%patterns.length],meaningOptions=(item,tag)=>paperAdvancedMeaningOptions(item.word,item.meaning,allCourseWords,meanings,tag),englishOptions=(answer,pool,tag)=>paperAdvancedEnglishOptions(answer,pool,tag),sentenceOptions=(answer,pool,tag)=>paperAdvancedSentenceOptions(answer,pool,tag),patternMeaningOptions=(item,tag)=>{const similar=paperAdvancedSentenceOptions(item.en,patternEn,tag).map(sentence=>allCoursePatterns.find(pattern=>pattern.en===sentence)?.zh||paperExactMeaning(sentence)).filter(Boolean);return paperShuffle(paperUnique([item.zh,...similar]).slice(0,3),`${tag}:zh`);};
    ...
    for(let index=0;index<5;index+=1){const item=patternAt(index+7);questions.push({section:"第五部分　听句理解",type:"听句选义",audio:item.en,q:"听完整句子，选择最符合的中文意思。",options:patternMeaningOptions(item,`${seed}:listen-sentence:${index}`),answer:item.zh,sourceSentence:item.en,targetZh:item.zh,why:`录音句子是“${item.en}”，完整意思是“${item.zh}”。先抓人物和动作，再检查细节。`});}
    for(let index=0;index<5;index+=1){const item=wordAt(index+24);questions.push({section:"第六部分　易混词辨析",type:"近形词辨析",q:`哪一个单词能准确表示“${item.meaning}”？`,options:paperAdvancedEnglishOptions(item.word,spellings,`${seed}:confusable:${index}`),answer:item.word,focusWord:item.word,focusMeaning:item.meaning,why:`正确拼写是 ${item.word}。要逐个比较首字母、元音和词尾，不能只凭“看起来眼熟”。`});}
    for(let index=0;index<5;index+=1){const item=patternAt(index+12),answer=item.en;questions.push({section:"第七部分　句子结构",type:"语序与句型",q:`选择能准确表达“${item.zh}”的完整句子。`,options:paperAdvancedSentenceOptions(answer,patternEn,`${seed}:order:${index}`),answer,sourceSentence:answer,targetZh:item.zh,why:`正确句子是“${answer}”。${item.rule||"英语句子要同时检查主语、动词和语序。"}`});}
    {const current=book.units[unitIndex],story=String(current.story||""),otherStories=COURSE_BOOKS.flatMap(item=>item.units).filter(item=>item!==current).map(item=>item.story),sentences=story.match(/[^.!?]+[.!?]?/g)?.map(item=>item.trim()).filter(Boolean)||[story],storyWords=new Set((story.toLowerCase().match(/[a-z]+/g)||[])),known=current.core.find(item=>storyWords.has(item.word.toLowerCase().split(" ")[0]))||current.core[0],otherWords=allCourseWords.filter(item=>!storyWords.has(item.word.toLowerCase().split(" ")[0]));
      questions.push({section:"第八部分　原创阅读理解",type:"阅读主旨",q:`阅读短文并选择最合适的内容概括：${story}`,options:paperOptions(current.zh,COURSE_BOOKS.flatMap(item=>item.units.map(unit=>unit.zh)),`${seed}:reading-main`),answer:current.zh,sourceSentence:story,targetZh:current.zh,why:`短文围绕“${current.zh}”展开。判断主旨要看全文反复出现的人物、动作和主题词。`});
      questions.push({section:"第八部分　原创阅读理解",type:"阅读辨词",q:`阅读短文：${story}　哪一个词或短语确实出现在文中？`,options:paperShuffle([known.word,...paperShuffle(otherWords,`${seed}:reading-words`).slice(0,2).map(item=>item.word)],`${seed}:reading-word-options`),answer:known.word,focusWord:known.word,focusMeaning:known.meaning,sourceSentence:story,why:`${known.word}（${known.meaning}）能在原文中找到。阅读时必须回到短文定位证据。`});
      const sentenceCount=sentences.length,countOptions=paperShuffle([String(sentenceCount),String(Math.max(1,sentenceCount-1)),String(sentenceCount+1)],`${seed}:reading-count`);questions.push({section:"第八部分　原创阅读理解",type:"阅读细节",q:`短文“${story}”共有几个完整句子？`,options:countOptions,answer:String(sentenceCount),sourceSentence:story,why:`按句号、问号或感叹号分句，共有 ${sentenceCount} 个完整句子。`});
      const trueSentence=sentences[0],falseSentences=paperShuffle(otherStories,`${seed}:reading-statements`).slice(0,2).map(text=>(String(text).match(/[^.!?]+[.!?]?/)||[text])[0].trim());questions.push({section:"第八部分　原创阅读理解",type:"阅读判断",q:`根据短文“${story}”，哪一句信息正确？`,options:paperShuffle([trueSentence,...falseSentences],`${seed}:reading-true`),answer:trueSentence,sourceSentence:story,why:`“${trueSentence}”直接来自原文，其余选项讲的是其他人物或情境。`});
      const firstWord=(story.match(/[A-Za-z]+/)||[""])[0],firstMeaning=paperTokenMeaning(firstWord);questions.push({section:"第八部分　原创阅读理解",type:"上下文理解",q:`在短文“${story}”中，开头的 ${firstWord} 指向什么信息？`,options:paperOptions(firstMeaning,meanings,`${seed}:reading-context`),answer:firstMeaning,sourceSentence:story,why:`结合句首和后面的谓语可知，${firstWord} 在这里表示“${firstMeaning}”。`});
    }
    if(type.id==="bridge-mock")BRIDGE_ABILITY_QUESTIONS.slice(0,10).forEach((item,index)=>questions.push({section:"第九部分　小升初语法衔接",type:item.area,q:item.q,options:item.options,answer:item.answer,why:item.why,difficultyTag:"小升初易错"}));
    ...
    return questions.slice(0,type.count||50);
  }
  const paperScoreKey=(book,unit,typeId)=>`${unit.id}:${typeId}`;
  function renderPapers(){
    const {book,unit}=paperSelection(),paperTypes=paperGrade===6?[...PAPER_TYPES,BRIDGE_PAPER_TYPE]:PAPER_TYPES,allScores=Object.entries(state.papers.scores).map(([key,value])=>({value:Number(value),count:key.endsWith(":bridge-mock")?60:50,pass:key.endsWith(":bridge-mock")?48:40})).filter(item=>Number.isFinite(item.value)),completedTotal=allScores.filter(item=>item.value>=item.pass).length,average=allScores.length?Math.round(allScores.reduce((sum,item)=>sum+item.value/item.count*100,0)/allScores.length):0,currentDone=paperTypes.filter(type=>Number(state.papers.scores[paperScoreKey(book,unit,type.id)]||0)>=type.pass).length,totalUnits=COURSE_BOOKS.reduce((sum,item)=>sum+item.units.length,0),bridgeUnits=COURSE_BOOKS.filter(item=>item.grade===6).reduce((sum,item)=>sum+item.units.length,0),totalPapers=totalUnits+bridgeUnits,totalQuestions=totalUnits*50+bridgeUnits*60;
    $("paperDashboard").innerHTML=`<article><span>📚</span><div><b>${totalUnits}</b><small>课程单元</small></div></article><article><span>📝</span><div><b>${totalPapers}</b><small>足量综合卷</small></div></article><article><span>✍️</span><div><b>${totalQuestions}</b><small>正式测评题量</small></div></article><article><span>✅</span><div><b>${completedTotal}</b><small>本身份已通过</small></div></article><article><span>🎯</span><div><b>${allScores.length?average+"%":"—"}</b><small>平均正确率</small></div></article>`;
    ...
    const totalCurrentQuestions=paperTypes.reduce((sum,type)=>sum+type.count,0);$("paperUnitSummary").innerHTML=`<div><span>${unit.icon}</span><div><small>${esc(book.label)} · UNIT ${unit.number} · ${esc(book.version.short)} · 原创组卷</small><h2>${esc(unit.title)} <i>${esc(unit.zh)}</i></h2><p>每单元保留一份足量综合卷，共${paperTypes.length}卷、${totalCurrentQuestions}题；参考福建公开卷常见题型结构，不复制或冒充学校真题。</p></div></div><aside><b>${currentDone}/${paperTypes.length}</b><small>正确率达到80%即通过</small><div><span style="width:${currentDone/paperTypes.length*100}%"></span></div></aside>`;
    $("paperGrid").innerHTML=paperTypes.map(type=>{const key=paperScoreKey(book,unit,type.id),score=Number(state.papers.scores[key]||0),attempts=Number(state.papers.attempts[key]||0),passed=score>=type.pass,scope=type.id==="bridge-mock"?"当前及全部过往知识":"当前单元完整知识";return `<article class="paper-card ${passed?"passed":""} ${type.id==="fujian-unit"?"public-combo":""} ${type.id==="bridge-mock"?"advanced-paper-card":""}"><div class="paper-card-top"><span>${type.icon}</span><div><small>${esc(type.kind)}</small><h3>${esc(type.title)}</h3></div><em>${esc(type.level)}</em></div><p>${esc(type.desc)}</p><div class="paper-card-facts"><span>${type.count}题</span><span>${type.minutes}分钟</span><span>${scope}</span><span class="advanced-fact">听力 · 易混词 · 句型 · 阅读 · 综合语境</span></div>${attempts?`<div class="paper-best"><b>最高 ${score}/${type.count}</b><small>已作答 ${attempts} 次</small></div>`:`<div class="paper-best empty"><b>尚未作答</b><small>完成全部${type.count}题后交卷</small></div>`}<button class="${passed?"soft":"primary"}" data-paper-start="${type.id}">${passed?"再次练习":"开始答卷"} →</button></article>`;}).join("");
    const workspace=$("paperExamWorkspace");
    if(!paperActiveId){workspace.hidden=true;workspace.innerHTML="";return;}
    const type=paperTypes.find(item=>item.id===paperActiveId);if(!type){paperActiveId="";workspace.hidden=true;return;}
  ...
  function renderPaperExam(book,unit,type){
    ...
    $("paperExamWorkspace").innerHTML=`<header class="paper-exam-head"><button class="paper-exam-close" data-paper-close aria-label="关闭试卷">×</button><div><small>${esc(book.label)} · Unit ${unit.number} · ${esc(type.kind)} · 原创组卷</small><h2>${type.icon} ${esc(type.title)}</h2><p>${scopeText} · 建议限时${type.minutes}分钟 · ${type.pass}/${total}通过</p></div><aside><b>${paperChecked?`${score}/${total}`:`${answered}/${total}`}</b><small>${paperChecked?(passed?"已通过":"需要订正"):"已作答"}</small></aside></header>${refs}${sections}<footer class="paper-submit-row">${paperChecked?`<div><b>${passed?"🎉 本卷通过":"📌 还需继续巩固"}</b><p>本次 ${score}/${total}，历史最高 ${Math.max(best,score)}/${total}。${passed?"建议隔3天再做一次。":"请先阅读错题解析，再重新答卷；达到80%后才会记录通过与发放奖励。"}</p></div><button class="soft" data-paper-retry>重新答卷</button>`:`<div><b>答题进度 ${answered}/${total}</b><p>必须完成全部${total}题后才能交卷，避免漏题。</p></div><button class="primary" data-paper-submit ${answered<total?"disabled":""}>${answered<total?`还需完成 ${total-answered} 题`:"提交并查看解析"}</button>`}</footer>`;
  }
  function submitPaper(){
    const {book,unit}=paperSelection(),type=(paperGrade===6?[...PAPER_TYPES,BRIDGE_PAPER_TYPE]:PAPER_TYPES).find(item=>item.id===paperActiveId),questions=type?paperQuestionPool(book,paperUnitIndex,type):[];if(!type||Object.keys(paperAnswers).length<questions.length)return toast(`请先完成本卷全部${questions.length||type?.count||50}题`);
    const key=paperScoreKey(book,unit,type.id),score=questions.filter((item,index)=>paperAnswers[index]===item.answer).length,previous=Number(state.papers.scores[key]||0),firstPass=previous<type.pass&&score>=type.pass;paperChecked=true;state.papers.scores[key]=Math.max(previous,score);state.papers.attempts[key]=Number(state.papers.attempts[key]||0)+1;
  ...
  function completeStage(stage=state.stage){
    ...
    const score=Number(state.evidence[key]||0),requirements={words:16,patterns:0,dialogue:6,reading:3,practice:16},need=requirements[stage]||0;
    if(score<need){const hints={words:"请先完成20题单词填词测试，并至少答对16题",dialogue:"请至少完成6次整句或分角色点读",reading:"请至少听读3篇材料各一次",practice:"请先完成20题练习，并至少答对16题"};return toast(hints[stage]||"请先完成本模块的学习要求");}
    if(stage==="words"){const progress=wordRecordingStatus(unitNow());if(!progress.passed)return toast(`录音跟读还差 ${progress.total-progress.done} 次：${progress.missing.slice(0,2).join("、")}`);}
    if(stage==="patterns"){const progress=patternRecordingStatus(unitNow());if(!progress.passed)return toast(`重点句型录音还差 ${progress.total-progress.done} 次：${progress.missing.slice(0,2).join("、")}`);}
  ...
  function recordStageEvidence(stage,token,value=1){const key=stageKey(stage),seenKey=`${key}:seen`,seen=state.evidence[seenKey]||[];if(token&&!seen.includes(token)){seen.push(token);state.evidence[seenKey]=seen;state.evidence[key]=Math.max(Number(state.evidence[key]||0),seen.length,value);save();}else if(!token&&value>Number(state.evidence[key]||0)){state.evidence[key]=value;save();}}
  function doneButton(label="完成这一步"){ return `<div class="stage-finish"><p>达到本模块学习要求后才能记录完成并领取奖励，避免只点按钮跳过学习。</p><button class="primary" id="completeStageBtn">${state.stageDone.includes(stageKey(state.stage))?"✓ 已完成":label+" +1 ☀️"}</button></div>`; }

  function unitDepthPanel(u){
    const focusWords=u.core.slice(0,Math.min(6,u.core.length));
    const focusPatterns=u.patterns.slice(0,3);
    const formNotes=focusWords.map(word=>({word,profile:wordProfile(word)})).filter(item=>item.profile.forms).slice(0,3);
    return `<section class="unit-depth-panel"><div class="unit-depth-head"><span>DEEP LEARNING</span><h3>本单元深度学习地图</h3><p>先弄懂“词是什么意思”，再观察“为什么这样组句”，最后把知识换成自己的真实表达。下面内容不是简单背诵清单，而是本单元的理解重点。</p></div><div class="unit-depth-grid"><article><header><b>1</b><div><small>词汇概念</small><h4>核心词怎样分组</h4></div></header><div class="depth-word-cloud">${focusWords.map(word=>`<span><strong>${esc(word.word)}</strong><em>${esc(word.meaning)}</em><small>${esc(wordProfile(word).pos)}</small></span>`).join("")}</div><p>学习时把这些词按“人物 / 事物 / 动作 / 特征 / 时间地点”重新分类，比按课本顺序连续背诵更牢固。</p></article><article><header><b>2</b><div><small>句型逻辑</small><h4>句子为什么这样说</h4></div></header><div class="depth-pattern-list">${focusPatterns.map((pattern,index)=>`<section><span>${index+1}</span><div><strong>${esc(pattern.en)}</strong><em>${esc(pattern.zh)}</em><p>${esc(pattern.rule)}</p></div></section>`).join("")}</div></article><article><header><b>3</b><div><small>易错辨析</small><h4>词形与语序检查</h4></div></header><ul class="depth-mistakes">${formNotes.map(item=>`<li><strong>${esc(item.word.word)}</strong><span>${esc(item.profile.forms)}</span></li>`).join("")}<li><strong>语序</strong><span>先找主语，再找动词；疑问句还要检查助动词或 be 动词的位置。</span></li><li><strong>书写</strong><span>句首大写，专有名词大写，句末补全句号或问号。</span></li></ul></article><article><header><b>4</b><div><small>真实迁移</small><h4>离开提示也能表达</h4></div></header><ol class="depth-transfer"><li>从上面任选3个词，各说一个与自己有关的句子。</li><li>任选1个核心句型，替换人物、时间或地点，再说2个新句子。</li><li>用3—5句话完成“${esc(u.zh)}”微表达，至少包含一个问句和一个回答。</li><li>合上提示后再说一次，并检查词形、语序和发音。</li></ol><div class="depth-example"><b>示范起点</b><span>${esc(wordExample(focusWords[0],0,u))}</span></div></article></div></section>`;
  }

  function renderStage(){
    const u=unitNow(), box=$("lessonContent");
    if(state.stage==="overview") box.innerHTML=`<div class="content-head"><span>单元原创课件导学</span><h2>这个主题单元怎样学习？</h2><p>本单元建议分3—5天完成。每次只学一个模块，第二天先复习5分钟。请同时查看上方版本标注，并以学校实际发放教材为准。</p></div><div class="material-summary"><article><b>${u.core.length}</b><span>必备与拓展词</span></article><article><b>8</b><span>核心与迁移句式</span></article><article><b>3×6</b><span>情境对话句</span></article><article><b>3+20</b><span>分级阅读与练习</span></article></div><div class="textbook-plan">${lessonPlan(u).map(item=>`<button data-open-stage="${item.stage}"><span>${item.icon}</span><div><small>LESSON ${item.number} · ${item.time}</small><h3>${item.title}</h3><p>${item.detail}</p></div><em>开始学习 →</em></button>`).join("")}</div><h3 class="goal-title">学完本单元，我可以做到</h3><div class="objective-list"><article><b>我能听懂</b><p>在“${u.zh}”情境中听出关键词，判断人物在谈论什么。</p></article><article><b>我能开口</b><p>${u.goal}</p></article><article><b>我能读懂</b><p>读懂3篇由浅入深的原创材料，找到人物、地点、时间与主要信息。</p></article><article><b>我能写出</b><p>完成20题分层练习，并仿照重点句型独立写出5个句子。</p></article></div>${unitDepthPanel(u)}<div class="explain-card"><span>${u.icon}</span><div><h3>生活情境</h3><p>想一想：你在真实生活中什么时候会用到“${u.zh}”英语？先用中文说清楚，再尝试说出一个英文关键词。</p><strong>学习秘诀：理解意思 → 看例子 → 自己换词 → 离开提示再说一遍。</strong></div></div>${doneButton("我已经看懂学习路线")}`;
  ...
  function renderWordStage(box,u){
    const core=seededShuffle(u.core.slice(0,Math.min(8,u.core.length)),`${iso()}:${u.id}:core`),extra=seededShuffle(u.core.slice(8),`${iso()}:${u.id}:extra`),testPool=core.flatMap((word,index)=>wordSentenceStudies(word,index,u)),testItems=seededShuffle(testPool,`${iso()}:${u.id}:sentence-test`).slice(0,Math.min(20,testPool.length)),done=state.stageDone.includes(stageKey("words")),eligible=done||Number(state.evidence[stageKey("words")]||0)>=Math.ceil(testItems.length*.8),recording=wordRecordingStatus(u);
    let testRecorded=done;
    box.innerHTML=`<div class="content-head"><span>第2步 · 单词本</span><h2>必备单词：听懂、录音跟读，再进入测试</h2><p>每个必备单词必须完成2次录音跟读；单词下的3个例句各录音1次。录音次数与20题填词测试都达标后，才能完成本模块。</p></div><div class="lesson-record-overview"><div><span>🎙️</span><div><small>本单元录音进度</small><b>${recording.done}/${recording.total} 次</b></div></div><progress max="${recording.total}" value="${recording.done}"></progress><p>单词2次 × ${core.length}个；每词3个例句各1次。录音只保存在当前页面，不上传。</p></div><section id="wordLearningBlock"><div class="word-section-title"><h3>⭐ 本单元重要单词</h3><small>先听标准音 → 单词录2次 → 每个例句录1次 → 回听检查</small></div><div class="vocab-grid sentence-vocab-grid">${core.map((w,i)=>wordCard(w,i,u,"core")).join("")}</div>${extra.length?`<div class="word-section-title"><h3>🚀 拓展单词</h3><small>拓展词也可以录音巩固，但不计入本模块必做次数</small></div><div class="vocab-grid extra sentence-vocab-grid">${extra.map((w,i)=>wordCard(w,i+8,u,"extra")).join("")}</div>`:""}<div class="memory-method"><h3>录音回忆法</h3><ol><li>先听标准单词，再录音跟读2次</li><li>每个例句听懂后完整录音1次</li><li>回听自己的声音，检查关键词是否清楚</li><li>遮住单词，口头补全句子</li></ol></div><button class="primary word-test-start" id="startWordTest">完成录音学习后，进入20题填词 →</button></section><section class="cloze-practice word-cloze-zone" id="wordClozeTest" hidden><div class="cloze-section-head"><div><h3>本单元单词填词测试</h3><p>单词和例句已经隐藏。请根据句意和中文提示，把学过的重要单词填回完整句子。全部作答且答对至少80%才通过。</p></div><span>${testItems.length} 题</span></div><div class="cloze-question-list">${testItems.map((item,index)=>`<article class="cloze-question" data-word-cloze="${index}"><span>${index+1}</span><div><b>${esc(blankSentence(item.en,item.answer))}</b><p>${esc(item.zh)}</p><small>提示：${esc(item.word.meaning)} · ${esc(item.answer.charAt(0))}${"＿".repeat(Math.max(1,item.answer.length-1))}</small><input type="text" autocomplete="off" spellcheck="false" data-word-cloze-answer data-word="${esc(item.word.word)}" data-expected="${esc(item.answer)}" placeholder="填入本单元单词"><em></em></div><button type="button" data-say="${esc(blankSentence(item.en,item.answer).replace("________","blank"))}" aria-label="朗读填空句">🔊</button></article>`).join("")}</div><div class="cloze-actions"><button class="soft" id="backToWordLearning">返回复习单词、例句与录音</button><button class="primary" id="checkWordCloze">提交并检查</button></div><div id="wordClozeResult"></div><div id="wordStageFinish" ${eligible?"":"hidden"}>${doneButton("我已完成录音、单词与句式学习")}</div></section>`;
    ...
    $("checkWordCloze").onclick=()=>{let correct=0,filled=0;box.querySelectorAll("[data-word-cloze-answer]").forEach(input=>{if(input.value.trim())filled+=1;const row=input.closest(".cloze-question"),expected=input.dataset.expected,word=input.dataset.word,ok=answerNorm(input.value)===answerNorm(expected),key=`${u.id}:${word}`;row.classList.remove("correct","wrong");row.classList.add(ok?"correct":"wrong");row.querySelector("em").textContent=ok?"✓ 正确":input.value.trim()?`正确答案：${expected}`:`还没有填写，正确答案：${expected}`;scheduleReview(key,ok,"cloze");if(ok){correct+=1;if(!state.mastered.includes(key))state.mastered.push(key);state.weak=state.weak.filter(item=>item!==key);}else if(!state.weak.includes(key))state.weak.push(key);});if(!testRecorded){state.quiz.correct+=correct;state.quiz.total+=testItems.length;testRecorded=true;}if(filled===testItems.length&&correct>=Math.ceil(testItems.length*.8))recordStageEvidence("words",null,correct);else save();const passed=filled===testItems.length&&correct>=Math.ceil(testItems.length*.8);$("wordClozeResult").innerHTML=`<div class="quiz-result"><b>${correct}/${testItems.length}</b><p>${passed?"达到80%通过线！请订正错词，然后记录本模块完成。":filled<testItems.length?`还有 ${testItems.length-filled} 题未填写，请先独立完成全部题目。`:"暂未达到80%。请读一遍正确句子，修改错题后重新检查。"}</p></div>`;$("wordStageFinish").hidden=!passed;$("checkWordCloze").textContent="再次检查";const complete=$("completeStageBtn");if(complete)complete.onclick=()=>completeStage("words");};
  }
  ...
  function wordProfile(w){
    ...
    return{pos,forms,meanings:unique.slice(0,6),reviewed:Boolean(override)};
  }
  function wordCard(w,i,u,type){
    const known=state.mastered.includes(`${u.id}:${w.word}`),sentences=wordSentenceStudies(w,i,u),profile=wordProfile(w),required=type==="core";
    return `<article class="vocab-card sentence-word-card ${known?"known":""}"><button class="sound" data-say="${esc(w.word)}">🔊 单词</button><small>${type==="core"?"必备":"拓展"} ${i+1}</small><h3>${esc(w.word)}</h3>${required?lessonRecordButton(`word:${w.word}`,2,`单词 ${w.word}`,600):""}<button class="meaning-cover" data-toggle-word>点击查看意思</button><p class="word-meaning">${esc(w.meaning)}</p><div class="word-profile"><div><b>词性 <em class="profile-review ${profile.reviewed?"reviewed":"pending"}">${profile.reviewed?"已人工复核":"规则推断·待复核"}</em></b><span>${esc(profile.pos)}</span></div><div><b>单复数 / 词形</b><span>${esc(profile.forms)}</span></div><div class="word-profile-meanings"><b>本课词义与常见义</b><span>${profile.meanings.map(meaning=>`<em>${esc(meaning)}</em>`).join("")}</span></div></div><div class="word-sentence-module"><div class="word-sentence-title"><em>句式学习</em><b>3个例句${required?" · 每句录音1次":" · 拓展理解"}</b></div><div class="word-sentence-list">${sentences.map((sentence,index)=>`<section><div><span>例句 ${index+1}</span><button data-say="${esc(sentence.en)}">🔊 整句</button></div>${interactiveSentence(sentence.en,u)}${required?lessonRecordButton(`word-example:${w.word}:${index}`,1,`${w.word} 的例句${index+1}`,1100):""}<div class="sentence-explanation"><p><b>句子意思</b>${esc(sentence.zh)}</p><p><b>${esc(sentence.role.label)}</b><strong>${esc(w.word)}</strong>：${esc(sentence.role.detail)}</p></div></section>`).join("")}</div></div></article>`;
  }
  ...
  function expandedPatternItems(u){
    ...
    u.patterns.forEach(pattern=>add({...pattern,kind:"本单元核心句型",focus:"理解结构后替换关键词"}));
    u.core.slice(0,8).forEach((word,index)=>wordSentenceStudies(word,index,u).forEach(study=>add({en:study.en,zh:study.zh,rule:`${study.role.label}：${study.role.detail}`,kind:"原创扩展例句",focus:`重点观察 ${word.word} 在完整句中的位置和词形`})));
    return items.slice(0,8);
  }
  function renderPatternStage(box,u){
    const patterns=expandedPatternItems(u),recording=patternRecordingStatus(u);
    box.innerHTML=`<div class="content-head"><span>第3步 · ${patterns.length}个完整句式</span><h2>重点句型：每句录音跟读3次，真正读顺再完成</h2><p>前${u.patterns.length}句是本单元核心句型，其余为本站原创扩展例句。每句先听标准音，再完成3次真实录音；录音不足不能完成本模块。</p></div><div class="lesson-record-overview"><div><span>🎙️</span><div><small>重点句型录音进度</small><b>${recording.done}/${recording.total} 次</b></div></div><progress max="${recording.total}" value="${recording.done}"></progress><p>${patterns.length}个重点句式，每句至少录音3次。建议三次分别检查发音、连贯和语调。</p></div><div class="module-volume"><article><b>${u.patterns.length}</b><span>核心句型</span></article><article><b>${patterns.length-u.patterns.length}</b><span>原创扩展例句</span></article><article><b>${patterns.length*3}</b><span>必做录音次数</span></article><article><b>1</b><span>独立表达任务</span></article></div><div class="pattern-list expanded-pattern-list">${patterns.map((p,i)=>`<article class="pattern-card"><div class="pattern-number">${i+1}</div><div><div class="pattern-kind">${esc(p.kind)} <span>${esc(p.focus||"")}</span></div><button class="line-sound" data-say="${esc(p.en)}">🔊 听完整句子</button>${interactiveSentence(p.en,u)}${lessonRecordButton(`pattern:${i}:${p.en}`,3,`重点句型${i+1}`,1200)}<p class="sentence-translation">${esc(p.zh)}</p><div class="rule"><b>为什么这样说？</b>${esc(p.rule)}</div><div class="try"><b>三次录音重点</b><span>第1次读清关键词；第2次减少停顿；第3次模仿整句语调，并尝试替换自己的真实信息。</span></div></div></article>`).join("")}</div><div class="mistake-box"><h3>⚠️ 本单元检查清单</h3><ul><li>句子开头是否大写？结尾是否有问号或句号？</li><li>主语换成 he / she 后，动词是否需要变化？</li><li>能否不看提示，用其中两个句式表达自己的真实信息？</li></ul></div>${doneButton("我已完成全部句型录音并会替换")}`;
    box.querySelectorAll("[data-say]").forEach(b=>b.onclick=()=>speak(b.dataset.say));
    bindSentenceWords(box);
    bindLessonRecorders(box);
  }
  ...
  function dialogueScenes(u){
    const curated=window.QUALITY_G3A_CONTENT?.[u.id]?.dialogues;
    if(Array.isArray(curated)&&curated.length)return curated;
    const p=u.patterns,w0=u.core[0],w1=u.core[1]||u.core[0];
    return [
      {title:"场景一 · 单元主题交流",tip:"先完整听一遍，再分角色朗读。",lines:dialogueLines(u)},
  ...
  function renderReadingStage(box,u){
    ...
    box.querySelectorAll("[data-say]").forEach((button,index)=>button.onclick=()=>{recordStageEvidence("reading",`reading:${index}`);speak(button.dataset.say);});
  }
  ...
  function quizItems(u){
    const curated=window.QUALITY_G3A_CONTENT?.[u.id]?.practice;
    if(Array.isArray(curated)&&curated.length===20)return curated;
  ...
  function renderPracticeStage(box,u){
    ...
    $("submitQuiz").onclick=()=>{
      if(Object.keys(quizAnswers).length<items.length)return toast(`请先独立完成全部20题，还差 ${items.length-Object.keys(quizAnswers).length} 题`);
      let correct=0; items.forEach((item,i)=>{const el=box.querySelector(`[data-question="${i}"]`), chosen=quizAnswers[i]; const ok=chosen===item.answer; if(ok)correct++; el.classList.remove("correct","wrong");el.classList.add(chosen?(ok?"correct":"wrong"):"wrong"); el.querySelector("p").innerHTML=chosen?(ok?`<b>✓ 回答正确</b>${esc(item.why)}`:`<b>正确答案：${esc(item.answer)}</b>${esc(item.why)}`):`<b>尚未作答，正确答案：${esc(item.answer)}</b>${esc(item.why)}`; const key=`${u.id}:${item.word}`;scheduleReview(key,ok,"unit-practice");if(ok){if(!state.mastered.includes(key))state.mastered.push(key);state.weak=state.weak.filter(x=>x!==key);}else if(!state.weak.includes(key))state.weak.push(key);});
      state.quiz.correct+=correct;state.quiz.total+=items.length;const passed=correct>=16,bonusKey=`practice-pass:${u.id}`;if(passed){recordStageEvidence("practice",null,correct);if(!state.dailyDone.includes(todayKey("quiz")))state.dailyDone.push(todayKey("quiz"));if(!state.bonuses.includes(bonusKey)){state.bonuses.push(bonusKey);reward(4,`通过20题分层练习，答对 ${correct} 题`,1);}else save();}else save();$("quizResult").innerHTML=`<div class="quiz-result"><b>${correct}/${items.length}</b><p>${correct===items.length?"全部正确！请隔3天再做一次，检查是否真正记牢。":passed?"已达到80%通过线。请重点朗读做错的句子，再完成真实表达。":correct>=12?"尚未达到16题通过线，请回到相应模块订正后再试。":"建议先复习单词与句型，再重新完成20题。"}</p></div>`;
    };
  }
  ...
  function renderDailyPractice(){
    const zhDone=state.dailyDone.includes(todayKey("zh2en")),enDone=state.dailyDone.includes(todayKey("en2zh")),reviewDone=state.dailyDone.includes(todayKey("review")),learned=pastLearnedWords(),due=dueReviewWords();
    $("dailyPracticeZone").innerHTML=`<article class="practice-launch ${zhDone?"done":""}"><span>✍️</span><small>独立窗口 A</small><h2>看中文，写英文</h2><p>窗口内只显示中文题目，不出现英文单词表，避免从旁边抄写。</p><button class="primary" data-open-dictation="zh2en">${zhDone?"再次练习":"开始英文默写"}</button></article><article class="practice-launch ${enDone?"done":""}"><span>🀄</span><small>独立窗口 B</small><h2>看英文，写中文</h2><p>窗口内只显示英文题目，不出现中文单词表，两种练习互不展示答案。</p><button class="primary" data-open-dictation="en2zh">${enDone?"再次练习":"开始中文释义"}</button></article><article class="practice-launch review-launch ${reviewDone?"done":""} ${learned.length?"":"locked"}"><span>🔁</span><small>间隔复习 · 今日到期 ${due.length} 词 · 已学 ${learned.length} 词</small><h2>按记忆周期复习过往单词</h2><p>${learned.length?"优先抽取今天到期和曾经答错的词；答对后会在1、3、7、15、30天后再次出现，绝不抽取未来课程。":"完成任意单元的“必备单词”学习后自动解锁。"}</p><button class="primary" data-open-review ${learned.length?"":"disabled"}>${reviewDone?"再次巩固":learned.length?"开始到期复习":"尚未解锁"}</button></article>`;
  }
  function openReview(){
    const learned=pastLearnedWords(),due=dueReviewWords(),words=seededShuffle(due.length?due:learned,`${iso()}:past-review`).slice(0,5),dialog=$("practiceDialog");
  ...
  function checkPastReview(){
    ...
    inputs.forEach(input=>{const answer=clean(input.value),expected=clean(input.dataset.expected),choices=expected.split(/或|\/|；/).map(clean),ok=input.dataset.mode==="zh2en"?answer===expected:choices.some(item=>item===answer||item.includes(answer)&&answer.length>=2),label=input.closest("label"),key=`${input.dataset.unitId}:${input.dataset.word}`;label.classList.toggle("correct",ok);label.classList.toggle("wrong",!ok);input.nextElementSibling.textContent=ok?"✓ 正确":`答案：${input.dataset.expected}`;scheduleReview(key,ok,"review");if(ok){correct+=1;state.weak=state.weak.filter(item=>item!==key);}else if(!state.weak.includes(key))state.weak.push(key);});
  ...
  function checkDictation(mode){
    ...
    inputs.forEach(input=>{const answer=clean(input.value),expected=clean(input.dataset.expected);const choices=expected.split(/或|\/|；/).map(clean);const ok=mode==="zh2en"?answer===expected:choices.some(item=>item===answer||item.includes(answer)&&answer.length>=2),key=`${unitKey()}:${input.dataset.word}`;input.closest("label").classList.toggle("correct",ok);input.closest("label").classList.toggle("wrong",!ok);input.nextElementSibling.textContent=ok?"✓ 正确":`答案：${input.dataset.expected}`;scheduleReview(key,ok,mode);if(ok){correct+=1;state.weak=state.weak.filter(item=>item!==key);}else if(!state.weak.includes(key))state.weak.push(key);});
  ...
  function wordPool(){
    ...
    if(memoryFilter==="bridge"){const ids=new Set(COURSE_BOOKS.filter(book=>book.grade===6).flatMap(book=>book.units.map(unit=>unit.id)));return seededShuffle(allWords().filter(word=>ids.has(word.unitId)),`${iso()}:bridge-wordbook`).map(word=>({...word,key:`${word.unitId}:${word.word}`}));}
  ...
  function moveWord(known){ const pool=wordPool(); if(!pool.length)return; const w=pool[memoryIndex];scheduleReview(w.key,known,"wordbook");if(known){if(!state.mastered.includes(w.key))state.mastered.push(w.key);state.weak=state.weak.filter(x=>x!==w.key);toast(`已记住：${state.reviewSchedule[w.key].due} 再回忆`);}else{if(!state.weak.includes(w.key))state.weak.push(w.key);state.mastered=state.mastered.filter(x=>x!==w.key);toast("已加入明日复习清单，慢慢来");} activity();save();memoryIndex=(memoryIndex+1)%Math.max(1,pool.length);memoryFlipped=false;renderWords(); }
  ...
  function renderBridgeAbility(){
    ...
    $("abilityWorkspace").innerHTML=`<header class="ability-module-head bridge-ability-head"><div><span>🎓</span><small>PRIMARY TO JUNIOR HIGH CHECK</small><h2>小升初20题综合检测</h2><p>覆盖近形词、冠词名词、代词、四大时态、搭配、比较、语序、连接词、阅读和写作。必须读懂完整句子，不能只凭眼熟。</p></div><aside><b>${bridgeAbilityChecked?`${score}/20`:`${answered}/20`}</b><small>${bridgeAbilityChecked?(passed?"达到衔接线":"需要补漏"):"已作答"}</small></aside></header><div class="bridge-ability-note"><b>建议限时25分钟</b><span>每题只有一个最合适答案；交卷后逐题显示中文理由和知识归属。</span></div><div class="diagnostic-grid bridge-ability-grid">${BRIDGE_ABILITY_QUESTIONS.map((item,index)=>{const selected=bridgeAbilityAnswers[index],correct=selected===item.answer;return `<article class="diagnostic-question ${bridgeAbilityChecked?(correct?"correct":"wrong"):""}"><div><span>${index+1}</span><small>${esc(item.area)} · 小升初易错</small></div><h3>${esc(item.q)}</h3><div>${item.options.map(option=>`<button class="${selected===option?"selected":""} ${bridgeAbilityChecked&&option===item.answer?"answer":""}" data-bridge-ability-answer="${index}" data-answer-value="${esc(option)}" ${bridgeAbilityChecked?"disabled":""}>${esc(option)}</button>`).join("")}</div>${bridgeAbilityChecked?`<p><b>${correct?"✓ 回答正确":"正确答案："+esc(item.answer)}</b>${esc(item.why)}</p>`:""}</article>`}).join("")}</div><div class="ability-submit-row">${bridgeAbilityChecked?`<div class="bridge-result ${passed?"passed":"retry"}"><b>${score}/20 · ${passed?"达到小升初衔接线":"建议按错题类型补漏"}</b><p>${passed?"请把做错的题隔3天再做，并进入模拟卷检验稳定性。":"回到12周计划中对应知识周，学完规则后再重新检测。"}</p></div><button class="soft" data-bridge-ability-retry>重新检测</button><button class="primary" data-view="bridge">返回强化计划</button>`:`<button class="primary" data-bridge-ability-submit ${answered<BRIDGE_ABILITY_QUESTIONS.length?"disabled":""}>${answered<BRIDGE_ABILITY_QUESTIONS.length?`还需完成 ${BRIDGE_ABILITY_QUESTIONS.length-answered} 题`:"提交并查看逐题解析"}</button>`}</div>`;
  }
  function submitBridgeAbility(){
    if(Object.keys(bridgeAbilityAnswers).length<BRIDGE_ABILITY_QUESTIONS.length)return toast("请先完成全部20题");
    bridgeAbilityChecked=true;const score=BRIDGE_ABILITY_QUESTIONS.filter((item,index)=>bridgeAbilityAnswers[index]===item.answer).length;state.bridge.abilityBest=Math.max(Number(state.bridge.abilityBest||0),score);
    if(score>=14)abilityReward("bridge-check",3,"通过小升初综合检测",2);else save();renderAbilities();
  }
  ...
  function submitAbilityListening(){
    ...
    const score=items.filter((item,index)=>abilityListeningAnswers[index]===item.answer).length,key=unitKey(),passed=score>=Math.ceil(items.length*.8),first=passed&&!state.abilities.listeningCompleted.includes(key);
    if(first){state.abilities.listeningCompleted.push(key);abilityReward(`listening:${key}`,2,`听力达到80%：${score}/${items.length}`,1);}else{save();if(!passed)toast(`本次 ${score}/${items.length}，达到 ${Math.ceil(items.length*.8)} 题才记录完成`);}renderAbilities();
  }
  ...
  async function startSpeakingRecording(index,button){
    ...
    try{
      ...
      speakingRecorder.onstop=()=>{const blob=new Blob(speakingChunks,{type:speakingRecorder.mimeType||"audio/webm"});if(speakingRecording)URL.revokeObjectURL(speakingRecording);speakingRecording=URL.createObjectURL(blob);const recordKey=`${unitKey()}:${index}`;speakingRecordedKeys.add(recordKey);result.innerHTML=`<audio controls playsinline src="${speakingRecording}"></audio><small>先完整听一遍，再和“标准句”比较。录音不会上传。</small>`;result.querySelector("audio").addEventListener("play",()=>{speakingPlayedKeys.add(recordKey);const completeButton=$("abilityWorkspace")?.querySelector(`[data-speaking-complete="${index}"]`);if(completeButton){completeButton.disabled=false;completeButton.textContent="已回听，完成这句";}});button.textContent="● 再录一次";button.classList.remove("recording");speakingStream?.getTracks().forEach(track=>track.stop());speakingStream=null;};
      speakingRecorder.start();
    }catch{toast("没有获得麦克风权限。请在浏览器地址栏的网站设置中允许麦克风。");}
  }
  function completeSpeaking(index){
    const key=`${unitKey()}:${index}`;if(state.abilities.speakingCompleted.includes(key))return;if(!speakingRecordedKeys.has(key))return toast("请先完成录音");if(!speakingPlayedKeys.has(key))return toast("请先点播放，完整回听自己的录音后再完成");state.abilities.speakingCompleted.push(key);abilityReward(`speaking:${key}`,1,"完成一句口语录音与回听");renderAbilities();
  }
  ...
  function renderPhonics(){
    ...
    document.querySelectorAll("#view-beginner [data-say-phoneme]").forEach(button=>button.onclick=()=>{phonemePlayedKeys.add(button.closest("[data-phoneme-key]")?.dataset.phonemeKey||"");speakPhoneme(button.dataset.sayPhoneme);});
    document.querySelectorAll("[data-finish-phoneme]").forEach(button=>button.onclick=()=>{const key=button.dataset.finishPhoneme;if(state.phonicsDone.includes(key)){toast("这个音标已经学过了，继续巩固吧");return;}if(!phonemePlayedKeys.has(key))return toast("请先播放标准音素，再对照口型跟读3遍");state.phonicsDone.push(key);reward(1,"完成一个音标跟读");renderBeginner();});
  }

  function renderGrammar(){
    const topics=window.GRAMMAR_TOPICS||[];if(!topics.length)return;
    const topic=topics.find(item=>item.id===grammarTopicId)||topics[0],questions=[...topic.quiz,...(GRAMMAR_EXTRA_QUESTIONS[topic.id]||[])];grammarTopicId=topic.id;
    ...
    const quizHtml=questions.map((item,index)=>{const selected=grammarAnswers[index],status=result?(result.answers[index]===item.answer?"correct":"wrong"):"";return "<article class=\"grammar-question "+status+"\" data-grammar-question=\""+index+"\"><div><span>"+(index+1)+"</span><h4>"+esc(item.q)+"</h4></div><div class=\"grammar-options\">"+item.options.map(option=>"<button class=\""+(selected===option?"selected ":"")+(result&&option===item.answer?"answer":"")+"\" data-grammar-option=\""+index+"\" data-grammar-value=\""+esc(option)+"\">"+esc(option)+"</button>").join("")+"</div>"+(result?"<p>"+(result.answers[index]===item.answer?"✓ 回答正确":"正确答案："+esc(item.answer))+"。"+esc(item.explain)+"</p>":"")+"</article>";}).join("");
    $("grammarClassroom").innerHTML="<header class=\"grammar-hero\"><div><span>"+topic.icon+"</span><small>GRAMMAR TOPIC</small><h2>"+esc(topic.title)+"</h2><p>"+esc(topic.intro)+"</p></div><aside><b>"+(passed?"✓ 已完成":best?"最好 "+best+"/"+questions.length:"尚未测试")+"</b><small>练习 "+attempts+" 次</small></aside></header><div class=\"grammar-learning-path\"><span><b>1</b>看规则</span><i>→</i><span><b>2</b>读例句</span><i>→</i><span><b>3</b>找原因</span><i>→</i><span><b>4</b>做检查</span></div><div class=\"grammar-section-list\">"+sectionHtml+"</div><section class=\"grammar-quiz\"><div class=\"grammar-quiz-head\"><div><span>CHECK YOURSELF</span><h3>课后理解检查</h3><p>先独立思考，再选择答案。达到80%才完成本主题，错题会显示原因。</p></div><b>"+questions.length+"题</b></div><div class=\"grammar-question-list\">"+quizHtml+"</div><button class=\"primary grammar-submit\" id=\"submitGrammarQuiz\">"+(result?"修改答案后再次检查":"提交答案")+"</button>"+(result?"<div class=\"grammar-result "+(result.passed?"passed":"retry")+"\"><span>"+(result.passed?"🎉":"🌱")+"</span><div><b>"+result.score+" / "+questions.length+"</b><p>"+(result.passed?"已完成本主题！请大声读一遍做错的句子。":"尚未达到80%。读完错题原因，再修改答案重新提交。")+"</p></div></div>":"")+"</section>";
  ...
  function submitGrammarQuiz(){
    ...
    const questions=[...topic.quiz,...(GRAMMAR_EXTRA_QUESTIONS[topic.id]||[])];if(questions.some((_,index)=>!Object.hasOwn(grammarAnswers,index)))return toast("请先完成全部题目，再提交检查");
    const answers={...grammarAnswers},score=questions.filter((item,index)=>answers[index]===item.answer).length,passed=score/questions.length>=.8,firstPass=passed&&!state.grammar.completed.includes(topic.id);
  ...
  function renderReport(){
    const accuracy=state.quiz.total?Math.round(state.quiz.correct/state.quiz.total*100):0,diagnostic=diagnosticResult(),dueCount=dueReviewWords().length; $("reportCards").innerHTML=`<article><span>📚</span><b>${completedUnits()}</b><small>完成单元</small></article><article><span>🧩</span><b>${state.stageDone.length}</b><small>完成学习步骤</small></article><article><span>🔤</span><b>${state.mastered.length}</b><small>掌握单词</small></article><article><span>🔁</span><b>${dueCount}</b><small>今日到期复习</small></article><article><span>🎯</span><b>${accuracy||"—"}${accuracy?"%":""}</b><small>小测正确率</small></article><article><span>🧭</span><b>${diagnostic?diagnostic.recommendedGrade+"年级":"—"}</b><small>诊断建议起点</small></article><article><span>🔡</span><b>${state.abilities.phonicsCompleted.length}/${NATURAL_PHONICS_LESSONS.length}</b><small>拼读闯关</small></article><article><span>🥣</span><b>${state.foods}</b><small>粮食库存</small></article><article><span>🐾</span><b>${state.pets.owned.length}</b><small>动物伙伴</small></article>`;
    ...
    const advice=[]; if(!diagnostic)advice.push("先完成一次入学诊断，找到不太难也不太简单的学习起点。");else if(state.abilities.phonicsCompleted.length<NATURAL_PHONICS_LESSONS.length)advice.push(`诊断建议从${diagnostic.recommendedGrade}年级基础开始；自然拼读已完成 ${state.abilities.phonicsCompleted.length}/${NATURAL_PHONICS_LESSONS.length} 关，可每次练1关。`);if(dueCount)advice.push(`今天有 ${dueCount} 个词到达复习时间。系统会优先挑5个，答对后自动延长到下一次复习。`);else if(state.weak.length)advice.push(`目前有 ${state.weak.length} 个错词，但今天未必全部到期；按每日任务中的间隔复习安排即可。`);else advice.push("目前没有积累错词。完成一次单元小测后，系统会给出更准确的复习建议。"); if(streakCount()<3)advice.push("先把目标定为连续3天，每天20分钟；形成节奏比一次学一小时更重要。"); else advice.push(`已经连续学习 ${streakCount()} 天。请多肯定孩子的坚持，不只看分数。`); advice.push("家长可以做听众：请孩子用本单元句型介绍一件真实的事，听懂后追问一个简单问题。"); $("parentAdvice").innerHTML=advice.map((a,i)=>`<article><span>${i+1}</span><p>${a}</p></article>`).join("");
  }
  ...
  $("closeParentDialog").onclick=closeParentDialog;
  $("parentDialog").addEventListener("click",event=>{if(event.target===$("parentDialog"))closeParentDialog();});
  $("unlockParent").onclick=unlockParentManager;
  $("parentPinInput").addEventListener("keydown",event=>{if(event.key==="Enter")unlockParentManager();});
  $("exportLearningData").onclick=downloadLearningBackup;
  $("importLearningData").onchange=event=>importLearningBackup(event.target.files?.[0]);
  $("saveParentPin").onclick=saveNewParentPin;
  $("openMoreMenu").onclick=()=>{const dialog=$("moreMenuDialog");dialog.showModal?.();dialog.classList.add("open");};
  $("closeMoreMenu").onclick=()=>{const dialog=$("moreMenuDialog");dialog.close?.();dialog.classList.remove("open");};
  $("moreMenuDialog").addEventListener("click",event=>{if(event.target===$("moreMenuDialog"))$("closeMoreMenu").click();});
  ...
  $("downloadPhonemePack").onclick=downloadPhonemePack;
  $("testAudioButton").onclick=()=>speak("Welcome to Sunny English. Let's learn together.",.78);
  ...
  if("serviceWorker" in navigator) window.addEventListener("load",()=>navigator.serviceWorker.register("service-worker.js?v=41",{updateViaCache:"none"}).then(reg=>reg.update()).catch(()=>{}));
})();
