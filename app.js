(() => {
  ...
  const load = (profileId=activeUserId) => { try {
    ...
    return {...defaultState,...raw,suns:TEST_MODE?TEST_BALANCE:persistedEconomy.suns,foods:TEST_MODE?TEST_BALANCE:persistedEconomy.foods,evidence:{...(raw.evidence||{})},reviewSchedule:{...(raw.reviewSchedule||{})},plant,pets,quiz:{...defaultState.quiz,...(raw.quiz||{})},beginner,grammar,abilities,papers,bridge};
  ...
  function completeStage(stage=state.stage){
    ...
    const score=Number(state.evidence[key]||0),requirements={words:16,patterns:8,dialogue:6,reading:3,practice:16},need=requirements[stage]||0;
    if(score<need){const hints={words:"请先完成20题单词填词测试，并至少答对16题",patterns:"请至少点读8个不同句式或示范句",dialogue:"请至少完成6次整句或分角色点读",reading:"请至少听读3篇材料各一次",practice:"请先完成20题练习，并至少答对16题"};return toast(hints[stage]||"请先完成本模块的学习要求");}
  ...
  function renderPatternStage(box,u){
    ...
    box.innerHTML=`<div class="content-head"><span>第3步 · ${patterns.length}个完整句式</span><h2>重点句型：知道为什么，再学会替换</h2><p>前${u.patterns.length}句是本单元核心句型，其余为本站原创扩展例句。每句都能整句点读、逐词点读，并说明使用重点。</p></div><div class="module-volume"><article><b>${u.patterns.length}</b><span>核心句型</span></article><article><b>${patterns.length-u.patterns.length}</b><span>原创扩展例句</span></article><article><b>${patterns.length*3}</b><span>建议跟读次数</span></article><article><b>1</b><span>独立表达任务</span></article></div><div class="pattern-list expanded-pattern-list">${patterns.map((p,i)=>`<article class="pattern-card"><div class="pattern-number">${i+1}</div><div><div class="pattern-kind">${esc(p.kind)} <span>${esc(p.focus||"")}</span></div><button class="line-sound" data-say="${esc(p.en)}">🔊 听完整句子</button>${interactiveSentence(p.en,u)}<p class="sentence-translation">${esc(p.zh)}</p><div class="rule"><b>为什么这样说？</b>${esc(p.rule)}</div><div class="try"><b>三遍学习法</b><span>第1遍听懂意思；第2遍逐词跟读；第3遍换成本单元其他词或自己的真实信息。</span></div></div></article>`).join("")}</div><div class="mistake-box"><h3>⚠️ 本单元检查清单</h3><ul><li>句子开头是否大写？结尾是否有问号或句号？</li><li>主语换成 he / she 后，动词是否需要变化？</li><li>能否不看提示，用其中两个句式表达自己的真实信息？</li></ul></div>${doneButton("我已会读并替换8个句式")}`;
    box.querySelectorAll("[data-say]").forEach((b,index)=>b.onclick=()=>{recordStageEvidence("patterns",`pattern:${index}:${b.dataset.say}`);speak(b.dataset.say);});
    bindSentenceWords(box);
  }
  ...
  if("serviceWorker" in navigator) window.addEventListener("load",()=>navigator.serviceWorker.register("service-worker.js?v=40",{updateViaCache:"none"}).then(reg=>reg.update()).catch(()=>{}));
})();
